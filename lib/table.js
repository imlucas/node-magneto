...
var common = require('./common'),
    ...
    util = require('util'),
    debug = require('debug')('magneto:table');
...
function Table(data){
    ...
    this.attributes = data.AttributeDefinitions;
}

util.inherits(Table, require('events').EventEmitter);
...
Table.prototype.validateLimit = function(data){
    if(data.Limit && data.Limit <= 0){
        throw new errors.ValidationException("Limit failed to satisfy constraint: Member must have value greater than or equal to 1");
    }
};
...
Table.prototype.returnValues = function(data, oldItem, newItem){
    ...
    case 'UPDATED_OLD':
        ...
        Object.keys(oldHash).forEach(function(key){
            if(updatedKeys.indexOf(key) > -1){
                result[key] = {};
                result[key][oldItem.attributes[key].type] = oldItem.attributes[key].value;
                return key;
            }
        });
        break;
    case 'UPDATED_NEW':
        ...
        Object.keys(newHash).forEach(function(key){
            if(updatedKeys.indexOf(key) > -1){
                result[key] = {};
                result[key][newItem.attributes[key].type] = newItem.attributes[key].value;
            }
        });
        break;
    case 'NONE':
    ...
    Object.keys(conditions).forEach(function(name){
        ...
        if(!value){
            if(exist === undefined){
                ...
                if(oldItem && oldItem.attributes[name]){
                    throw new errors.ConditionCheckFailedException('A condition specified in the operation could not be evaluated.');
                }
            }
        }
        else{
            var expectedAttr = Attribute.fromHash(name, value);
            if((exist === undefined || exist === true) && !oldItem.attributes[name].equals(expectedAttr)){
                throw new errors.ConditionCheckFailedException('A condition specified in the operation could not be evaluated.');
            }
            else if(exist === false){
        ...
        return {
            'TableDescription': {
                ...
                'ProvisionedThroughput': {
                    ...
                    'LastDecreaseDateTime': this.lastIncreasedTime,
                    'LastIncreaseDateTime': this.lastDecreasedTime,
