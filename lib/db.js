...
var DB = {
    ...
    'createTable': function(data){
        ...
        if(tableItems[tableName]){
            table.items = tableItems[tableName];
        }
    ...
    'batchGetItem': function(data){
        ...
        Object.keys(data.RequestItems).forEach(function(tableName){
            ...
            if(!capacity.hasOwnProperty(tableName)){
                capacity[tableName] = {
                    TableName: tableName,
                    CapacityUnits: 1
                };
            ...
            tableData.Keys.forEach(function(key){
                ...
                if(itemHash){
                    response[tableName].push(itemHash);
        ...
        var capacityArray = [];
        Object.keys(capacity).forEach(function(tableName) {
            capacityArray.push(capacity[tableName]);
        });

        return {
            'ConsumedCapacity': capacityArray,
            'Responses': response
            // 'UnprocessedKeys': {}
        };
    },
